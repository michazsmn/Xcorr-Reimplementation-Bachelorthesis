...
\end{figure}
An example for a good match is shown in \cref{predandtheo}. The blue peaks are generated by the processed experimental spectrum from the scan 71120, the red peaks are the theoretical spectrum for the peptide "LAADEDDDDDDEEDDDEDDDDDDFDDEEAEEKAPVK". As one can see, most of the red theoretical spectrum peaks in \cref{predandtheo}(a) match the highest blue peaks. In this case, 60 of the total 140 theoretical spectrum ion peaks match a blue peak. 
...
\cref{predandtheocorr} demonstrates that the highest correlation was calculated at zero offset of both experimental and theoretical/predicted spectra. There are notably less background similarity peaks in the correlation array of the predicted spectrum, which demonstrates that the predicted spectrum is more accurate than the theoretical one.

All of this are high indicators that the experimental spectrum  with the scan number 71120 might indeed be produced by the peptide "LAADEDDDDDDEEDDDEDDDDDDFDDEEAEEKAPVK".
